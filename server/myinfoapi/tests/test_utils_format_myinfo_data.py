...
@pytest.mark.parametrize(
    "filename,expected", [
        (
            'myinfo_person_data_empty_cpfctrbn_history.json',
            {
                'uinfin': 'S9812387C',
                'name': 'HOWARD CHEN YU SHENG',
                'sex': 'MALE',
...
def test_format_myinfo_data__empty_cpfctrbn_history(filename, expected):
    """
    Should return without error the result with empty list of cpfcontributions
    """
    relpath = f"data/{filename}"
    filepath = Path(__file__).parent / relpath
    data = json.load(filepath.open())
...
@pytest.mark.parametrize(
    "filename,expected", [
        (
            'myinfo_person_data_empty_cpfbalances.json',
            {
                'uinfin': 'S9812387C',
                'name': 'HOWARD CHEN YU SHENG',
                'sex': 'MALE',
...
def test_format_myinfo_data_empty_cpfbalances(filename, expected):
    """
    Should return without error result with cpfbalannce account of 0
    """
    relpath = f"data/{filename}"
    filepath = Path(__file__).parent / relpath
    data = json.load(filepath.open())
